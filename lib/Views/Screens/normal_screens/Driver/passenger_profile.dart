...
import 'package:ponflow_carpoolingapp/Views/Screens/normal_screens/Driver/review_page.dart';
...
class PassengerProfile extends StatefulWidget {
  ...
  final Color color;
  final int index;
  const PassengerProfile({Key? key, required this.color,required this.index}) : super(key: key);
                ...
                onPressed: () {
                  if(widget.index>0) {
                    Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                          const ReviewPage(index: 1)
                        ));
                  }
                },

                child: (widget.index>0) ? const Text('Review', style: TextStyle(
                    color: Colors.white,
                    fontFamily: "SegoeUI",
                    fontWeight: FontWeight.bold,
                    fontSize: 20),): null,
